...
// Finglish vocab (kept on purpose - your own math words):
//   shoaa  = radius, ghotr = diameter (ghotr = 2 * shoaa),
//   mohit  = circumference (mohit = 2 * pi * shoaa),
//   masahat = area
void circleFacts(double shoaa) {
    const double pi = 3.14159;
    double ghotr = 2 * shoaa;                                             // diameter
    double masahat = pi * shoaa * shoaa;                                   // area
    double sphereVolume = (4.0 / 3.0) * pi * shoaa * shoaa * shoaa;        // FIXED: 4.0/3.0, not 4/3
    double sphereArea = 4 * pi * shoaa * shoaa;
    std::cout << "\n--- Circle/Sphere with shoaa (radius) " << shoaa << " ---" << std::endl;
    std::cout << "Ghotr (diameter) = " << ghotr << std::endl;
    std::cout << "Masahat (area) = " << masahat << std::endl;
